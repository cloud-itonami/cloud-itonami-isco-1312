(ns aquaculture-management.advisor
  "AquacultureAdvisor — proposes an aquaculture operation (schedule a harvest,
  log yield data, order supplies, report a stock/water-quality anomaly) for a
  registered aquaculture facility. The advisor is swappable: `mock-advisor`
  ...
  of site provenance or anomaly/supply risk; `aquaculture-management.governor`
  is the independent system that decides whether the proposal may
  proceed, per the itonami actor pattern.

  A proposal is a map:
    {:op :schedule-harvest|:log-yield-report|:order-supplies|:flag-stock-anomaly
...
(defn- infer
  ...
  {:op op
   ...
   :rationale (str "proposed " (name op) " for aquaculture facility " (:site-id request))})
...
(def ^:private system-prompt
  "You are an aquaculture management advisor. Given an aquaculture operation request,
