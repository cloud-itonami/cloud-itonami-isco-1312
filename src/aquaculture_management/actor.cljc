(ns aquaculture-management.actor
  "AquacultureActor — the ISCO-08 1312 aquaculture and fisheries production
  manager actor as a `langgraph.graph/state-graph` (per ADR-2607011000
  / CLAUDE.md Actors section). One graph run = one aquaculture operation
  ...
  The unconditional invariant: the AquacultureAdvisor can never
  directly commit a record or dispatch a robot action the
  AquacultureGovernor refuses — every commit-record! call is gated
  behind `:decide`."
...
(defn build-graph
  "Build a compiled AquacultureActor graph. `store` implements
  `aquaculture-management.store/Store`. `advisor` implements
  `aquaculture-management.advisor/Advisor` (defaults to `mock-advisor`).
